...
def soap_header(from_member_name, friendly_name, proxy, msnp_ver, build_ver,
    ...
    return """ 
      <From memberName="%(from_member_name)s" friendlyName="%(friendly_name)s"
            xml:lang="en-en" proxy="%(proxy)s" 
            xmlns="http://messenger.msn.com/ws/2004/09/oim/" 
            msnpVer="%s(msn_ver)s" buildVer="%(build_ver)s"/>
      <To memberName="%(to_member_name)s" 
          xmlns="http://messenger.msn.com/ws/2004/09/oim/"/>
      <Ticket passport="%(passport)s" appid="%(app_id)s" lockkey="%(lock_key)s" 
              xmlns="http://messenger.msn.com/ws/2004/09/oim/"/>
...
def soap_action():
    ...
    return "http://messenger.msn.com/ws/2004/09/oim/Store"
