...
cdef void _virtual_evas_position_get(Etk_Toplevel *toplevel, int *x, int *y):
    self = Object_from_instance(<Etk_Object *>toplevel)
    (x[0], y[0]) = self._evas_position_get()

cdef void _virtual_screen_position_get(Etk_Toplevel *toplevel, int *x, int *y):
    self = Object_from_instance(<Etk_Object *>toplevel)
    (x[0], y[0]) = self._screen_position_get()

cdef void _virtual_size_get(Etk_Toplevel *toplevel, int *w, int *h):
    self = Object_from_instance(<Etk_Object *>toplevel)
    (w[0], h[0]) = self._size_get()

cdef void _virtual_pointer_set(Etk_Toplevel *toplevel, Etk_Pointer_Type pointer_type):
    self = Object_from_instance(<Etk_Object *>toplevel)
...
cdef public class Toplevel(Bin) [object PyEtk_Toplevel, type PyEtk_Toplevel_Type]:
    cdef int _set_obj(self, Etk_Object *obj) except 0:
        ...
        if getattr(self.__class__, "_evas_position_get", None) is not None:
            t.evas_position_get = _virtual_evas_position_get

        if getattr(self.__class__, "_screen_position_get", None) is not None:
            t.screen_position_get = _virtual_screen_position_get

        if getattr(self.__class__, "_size_get", None) is not None:
            t.size_get = _virtual_size_get

        if getattr(self.__class__, "_pointer_set", None) is not None:
    ...
    def focused_widget_set(self, Widget widget):
        etk_toplevel_focused_widget_set(<Etk_Toplevel*>self.obj, <Etk_Widget*>widget.obj)
