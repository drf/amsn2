...
def set_pointer(user_data, pointer):
    if pointer == etk.Embed.POINTER_TEXT_EDIT:
...
entry1.item_prepend("First item")
...
entry2.item_append("2 item")
entry2.item_prepend("First item")
entry2.item_append("last item")
...
embed2.position_method_set(get_position, ecore_evas)
...
entry1.connect("active-item-changed", item_cb)
entry2.connect("active-item-changed", item_cb)
